...
let config;
try {
    const defaultConfig = JSON.parse(fs.readFileSync('./node_modules/create-react-component/.componentConfig.json', 'utf-8'))
    const userConfig = JSON.parse(fs.readFileSync('.componentConfig.json', 'utf-8'))
    config = {
        ...defaultConfig,
        ...userConfig
    }
    ...
    config = JSON.parse(fs.readFileSync('.componentConfig.json', 'utf-8'))
}
